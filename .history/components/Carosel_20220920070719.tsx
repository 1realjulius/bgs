import Image from "next/image";
import React, { useState } from "react";

const Carosel = () => {
  const Images = [
    { id: 1, image: "/Assets/bright-rain.png" },
    { id: 2, image: "/Assets/floss.png" },
    { id: 3, image: "/Assets/moonrise.png" },
    { id: 4, image: "/Assets/moonrise.png" },
  ];

  const [current, setCurrent] = useState(0);
  const slideLength = Images.length;

  if (!Array.isArray(Images) || Images.length <= 0) {
    return null;
  }

  const nextSlide = () => {
    setCurrent(current === slideLength - 1 ? 0 : current + 1);
  };
  const prevSlide = () => {
    setCurrent(current === 0 ? slideLength - 1 : current - 1);
  };

  return (
    <div>
      <div className="min-w-screen relative flex min-h-screen">
        {/* This is the image */}
        {Images.map((slide, index) => (
          <div
            className={
              index === current
                ? "slide active h-screen w-screen overflow-hidden object-cover transition duration-500"
                : "slide transition duration-100"
            }
            key={index}
          >
            {index === current && (
              <Image
                src={slide.image}
                // width={1080}
                // height={720}
                layout="fill"
                alt=""
                className=" absolute object-contain"
              />
            )}
          </div>
        ))}
        {/* the header */}
        <div className="absolute top-1">This is the carosel</div>
        {/* The btbs */}
        <div className="absolute bottom-0 right-0 flex flex-row items-center space-x-6">
          {/* Arrow Left */}
          <div>
            <button onClick={prevSlide}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="1.5"
                stroke="currentColor"
                className="h-12 w-12"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M11.25 9l-3 3m0 0l3 3m-3-3h7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                />
              </svg>
            </button>
          </div>
          {/* Arrow Right */}
          <div>
            <button onClick={nextSlide}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="1.5"
                stroke="currentColor"
                className="h-12 w-12"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12.75 15l3-3m0 0l-3-3m3 3h-7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                />
              </svg>
            </button>
          </div>
        </div>
        {/* The current Image Count */}
        <div className="absolute bottom-2">
          <h1>
            {current + 1}/{slideLength}
          </h1>
        </div>
      </div>
    </div>
  );
};

export default Carosel;
